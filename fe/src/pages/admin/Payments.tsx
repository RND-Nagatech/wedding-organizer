...
import { useClients, usePayments } from "@/lib/dataStore";
...
const Payments = ({
  ...
  const todayJakarta = useMemo(() => {
    // ISO date in Asia/Jakarta: YYYY-MM-DD
    return new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Jakarta" });
  }, []);

  const showMainTodayMode = !filterClientCode && !readOnly;

  const [page, setPage] = useState(1);
  const [perPage, setPerPage] = useState(10);

  const list = payments.filter((p) => {
    if (showMainTodayMode) {
      // Halaman utama pembayaran: tampilkan transaksi hari ini saja
      const paid = String(p.paidDate || "").slice(0, 10);
      if (!paid || paid !== todayJakarta) return false;
    } else {
      // Mode embedded: masih boleh filter berdasarkan client
  ...
  useMemo(() => { setPage(1); }, [kodeClient, status, q, perPage]);
  ...
  return (
    <>
      <PageHeader
        title="Pembayaran"
        subtitle={showMainTodayMode ? `${list.length} transaksi hari ini (${todayJakarta})` : `${list.length} transaksi pembayaran`}
        actions={
          readOnly ? undefined : (
            <PaymentFormDialog
              mode="add"
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-6">
        ...
        {/* NOTE: card "Sisa Tagihan" di-hide sesuai requirement (halaman utama pembayaran) */}
        {showMainTodayMode ? null : (
          <Card className="p-5 border-border shadow-soft bg-gradient-card">
      ...
      <Card className="border-border shadow-soft overflow-hidden">
        <div className="p-4 border-b border-border bg-muted/20">
          <div className="grid sm:grid-cols-4 gap-3">
            ...
            <div className={showMainTodayMode ? "space-y-1.5 sm:col-span-3" : "space-y-1.5 sm:col-span-2"}>
              <Label>Cari</Label>
            ...
            <TableBody>
              {pagedList.map((p) => (
                <TableRow key={p.id} className="hover:bg-muted/30 transition-smooth">
                  ...
                  <TableCell>{String(p.paidDate || "").slice(0, 10) || "—"}</TableCell>
