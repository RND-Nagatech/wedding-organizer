...
import { store, useAdat, useKatalogDekorasi, useKatalogFavorit } from "@/lib/dataStore";
...
export default function ClientCatalogDekorasi() {
  ...
  const favSet = useMemo(() => {
    const ids = favs
      .filter((f) => f.client_id === clientId && f.katalog_type === "dekorasi")
  ...
  }, [list, filterAdat, filterTema, filterWarna, q, filterMode, favSet]);
                  ...
                  <div key={x.id} className="relative">
                    ...
                    <button
                      ...
                      onClick={async (e) => {
                        ...
                        if (!clientId) return;
                        try {
                          const res = await store.toggleKatalogFavorit({ client_id: clientId, katalog_type: "dekorasi", katalog_id: String(x.id) });
          ...
          <DialogFooter>
            <Button
              ...
              onClick={async () => {
                ...
                if (!clientId) return;
                try {
                  const res = await store.toggleKatalogFavorit({ client_id: clientId, katalog_type: "dekorasi", katalog_id: String(selected.id) });
                  toast.success(res.action === "added" ? "Ditambahkan ke favorit" : "Dihapus dari favorit");
