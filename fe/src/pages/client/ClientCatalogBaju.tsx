...
import { store, useAdat, useKatalogBaju, useKatalogFavorit } from "@/lib/dataStore";
...
export default function ClientCatalogBaju() {
  ...
  const favSet = useMemo(() => {
    const ids = favs
      .filter((f) => f.client_id === clientId && f.katalog_type === "baju")
  ...
  }, [list, filterAdat, filterKategori, filterWarna, q, filterMode, favSet]);
                  ...
                  <div key={x.id} className="relative">
                    ...
                    <button
                      ...
                      onClick={async (e) => {
                        ...
                        if (!clientId) return;
                        try {
                          const res = await store.toggleKatalogFavorit({ client_id: clientId, katalog_type: "baju", katalog_id: String(x.id) });
          ...
          <DialogFooter>
            <Button
              ...
              onClick={async () => {
                ...
                if (!clientId) return;
                try {
                  const isFav = favSet.has(String(selected.id));
                  const res = await store.toggleKatalogFavorit({ client_id: clientId, katalog_type: "baju", katalog_id: String(selected.id) });
                  toast.success(res.action === "added" ? "Ditambahkan ke favorit" : "Dihapus dari favorit");
                  // keep modal open for browsing
                  if (!isFav && filterMode === "fav") {
                    // if we are in fav mode and user removed, close modal
                    setOpen(false);
                  }
                } catch (err: any) {
                  toast.error(err?.message || "Gagal update favorit");
